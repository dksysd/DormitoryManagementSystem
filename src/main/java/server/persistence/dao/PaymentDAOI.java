...
/**
 * PaymentDAOI 인터페이스는 데이터베이스에서 결제 데이터를 관리하기 위한 메서드를 정의합니다.
 * <p>
 * 이 인터페이스는 결제 데이터를 검색, 저장, 업데이트 및 삭제하는 기능을 제공합니다.
 */
public interface PaymentDAOI {

    /**
     * 주어진 ID를 기반으로 결제 데이터를 검색합니다.
     *
     * @param id 검색할 결제 데이터의 고유 ID
     * @return 주어진 ID에 해당하는 {@link PaymentDTO} 객체
     * @throws SQLException 데이터베이스에서 데이터를 검색하는 동안 오류가 발생한 경우
     */
    PaymentDTO findById(Integer id) throws SQLException;

    /**
     * 모든 결제 데이터를 검색하여 반환합니다.
     *
     * @return 데이터베이스에 저장된 모든 결제 데이터의 리스트
     * @throws SQLException 데이터베이스에서 데이터를 검색하는 동안 오류가 발생한 경우
     */
    List<PaymentDTO> findAll() throws SQLException;

    /**
     * 특정 사용자의 UID를 기반으로 결제 데이터를 검색합니다.
     *
     * @param uid 검색할 사용자의 UID (고유 식별자)
     * @return UID에 해당하는 {@link PaymentDTO} 객체
     * @throws SQLException 데이터베이스에서 데이터를 검색하는 동안 오류가 발생한 경우
     */
    PaymentDTO findByUid(String uid) throws SQLException;

    /**
     * 새로운 결제 데이터를 데이터베이스에 저장합니다.
     *
     * @param paymentDTO 저장할 {@link PaymentDTO} 객체
