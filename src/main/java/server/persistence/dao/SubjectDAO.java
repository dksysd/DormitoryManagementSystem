...
public class SubjectDAO implements SubjectDAOI {
    ...
    public SubjectDTO findById(Integer id) throws SQLException {
        String query = "SELECT id, subject_name, description, credit, created_at, updated_at, professor_id FROM subjects WHERE id = ?";
    ...
    public SubjectDTO findByName(String subjectName) throws SQLException {
        String query = "SELECT id, subject_name, description, credit, created_at FROM subjects WHERE subject_name = ?";
        try (Connection connection = DatabaseConnectionPool.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(query)) {
    ...
    public List<SubjectDTO> findAll() throws SQLException {
        ...
        String query = "SELECT id, subject_name, description, credit, created_at, updated_at, professor_id FROM subjects";
        try (Connection connection = DatabaseConnectionPool.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(query);
             ResultSet resultSet = preparedStatement.executeQuery()) {
    ...
    private SubjectDTO mapRowToSubjectDTO(ResultSet resultSet) throws SQLException {
        return SubjectDTO.builder()
                ...
                .professorId(new UserDTO(resultSet.getInt("professor_id"))) // ID만 세팅
                .build();
    }
}
